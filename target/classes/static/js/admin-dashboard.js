...
class AdminDashboard {
            ...
            row.innerHTML = `
                ...
                <td>
                    <div class="action-buttons-cell">
                        ...
                        ${this.renderActiveToggleButton(user)}
                    </div>
                </td>
            `;
    ...
    renderActiveToggleButton(user) {
        if (this.isAdminUser(user)) {
            return '';
        }
        const isActive = user.isActive === true || user.active === true;
        const icon = isActive ? 'bi-person-x' : 'bi-person-check';
        const title = isActive ? 'Disable User' : 'Enable User';
        const nextState = isActive ? 'false' : 'true';
        const colorClass = isActive ? 'btn-delete' : 'btn-view';
        return `
            <button class="btn btn-action ${colorClass}" 
                    onclick="adminDashboard.setUserActive(${user.id}, ${nextState})"
                    title="${title}">
                <i class="bi ${icon}"></i>
            </button>
        `;
    ...
    async setUserActive(userId, isActive) {
        try {
            const response = await fetch(`/api/admin/users/${userId}/active?active=${isActive}`, {
                method: 'PUT'
            });
            const result = await response.json();
            if (result.success) {
                this.showSuccess(isActive ? 'User enabled' : 'User disabled');
                await this.loadUsers();
            } else {
